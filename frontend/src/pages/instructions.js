      ...
      try {
        ...
        if (result.success) {
          setQuizState(result.data);
           ...
           const isQuizActive = result.data.isActive || result.data.round1?.isActive;
        ...
        <div className="max-w-4xl mx-auto">
             ...
             <div className={`mt-4 p-4 rounded-lg border ${
               ...
               <div className="flex items-center justify-center mb-2 space-x-2">
                 <div className={`w-3 h-3 rounded-full ${
                   quizState.isActive || quizState.round1?.isActive ? 'bg-green-400' : 'bg-blue-400'
                 }`}></div>
                 <span className="font-medium">
                   {quizState.isActive || quizState.round1?.isActive ? 'QUIZ IS ACTIVE!' : 'WAITING IN LOBBY'}
                 </span>
               </div>
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            >
              ...
              {/* Start Button */}
               <motion.button
                 whileHover={{ scale: (quizState.isActive || quizState.round1?.isActive) && acknowledged ? 1.05 : 1 }}
                 whileTap={{ scale: (quizState.isActive || quizState.round1?.isActive) && acknowledged ? 0.95 : 1 }}
                 onClick={startQuiz}
                 disabled={!acknowledged || !(quizState.isActive || quizState.round1?.isActive)}
